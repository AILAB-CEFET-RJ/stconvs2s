...
class SpatialBlock_NoChannelIncrease(nn.Module):
    def __init__(self, num_layers, kernel_size, in_channels, out_channels, dropout_rate):
        super(SpatialBlock_NoChannelIncrease, self).__init__()
        self.padding = kernel_size // 2
        self.dropout_rate = dropout_rate
        self.conv_layers = nn.ModuleList()
        self.dropout_layers = nn.ModuleList()
        ...
        for i in range(num_layers):
            self.conv_layers.append(
                nn.Sequential(
                    nn.Conv3d(in_channels, out_channels, kernel_size=spatial_kernel_size, 
                              padding=spatial_padding, bias=False),
